...
const employee_get_all = (req, res) => {
  Employee.find({ deleted: false }).sort({ createdAt: -1 })
    .then((result) => {
...
const employee_create = async (req, res) => {
  try{
    ...
    if(employee){
      await Employee.findOneAndUpdate({ email: employee.email }, { $set: req.body, deleted: false }, { new: true }, (err, result) => {
        if(err){
...
const employee_delete = async (req, res) => {
  try{
    await Employee.findOneAndUpdate({ employee_number: req.query.employee_number }, { deleted: true }, (err, result) => {
      if (err) {
