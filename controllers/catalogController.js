...
const handleProductErrors = (err) => {
  ...
  if (err.code === 11000) {
    ...
    if (err.message.includes('sku')) errors.sku = 'That SKU already exists!'
    return errors
  }
  //validation errors
...
const catalog_product_update = async (req, res) => {
  try {
    ...
    await CatalogProduct.findOneAndUpdate(
      ...
      (err, product) => {
        if (err) {
          console.warn(
            `An error occurred in catalog_product_update @ time: ${helpers.getTimeStamp()}`
          )
          console.log(`Error: ${err.message}`)
          res.status(400).json({
          ...
          console.log(
            `Product ${product._id} updated @ time: ${helpers.getTimeStamp()}`
          )
          res.status(200).json({
            message: 'Product updated!',
            catalog_prodcut: product._id,
          })
        }
      }
    )
  ...
  /* try {
    ...
    product.garage_product_number = req.body.garage_product_number
      ? req.body.garage_product_number
      : product.garage_product_number
...
const catalog_service_update = async (req, res) => {
  ...
  await CatalogService.findOneAndUpdate(
    ...
    (err, result) => {
      if (err) {
        console.warn(
          `An error occured in catalog_service_update @ time: ${helpers.getTimeStamp()}`
        )
        console.log(`Error: ${err.message}`)
        res.status(400).json({
          message: 'Unable to update serice!',
          error: err.message,
        })
      } else {
        console.log(
          `Catalog serivce ${
            result._id
          } updated @ time: ${helpers.getTimeStamp()}`
        )
        res.status(200).json({
          message: 'Catlog service Successfully updated!',
          catlogService: result._id,
        })
      }
    }
  )
  ...
  /*try {
    ...
    service.garage_service_number = req.body.garage_service_number
      ? req.body.garage_service_number
      : service.garage_service_number
    service.description = req.body.description
      ? req.body.description
      : service.description
    service.time_estimate = req.body.time_estimate
      ? req.body.time_estimate
      : service.time_estimate
    service.skill_level = req.body.skill_level
      ? req.body.skill_level
      : service.skill_level
    service.customer_note = req.body.customer_note
...
const catalog_product_delete = async (req, res) => {
  try {
    ...
    const product = await CatalogProduct.findByIdAndUpdate(req.query._id, {
      deleted: true,
    })
    product
      .save()
...
const catalog_service_delete = async (req, res) => {
  try {
    const service = await CatalogService.findByIdAndUpdate(req.query._id, {
      deleted: true,
    })
    service
      .save()
